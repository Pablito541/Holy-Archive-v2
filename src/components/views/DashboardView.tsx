...
export const DashboardView = ({ items, onViewInventory, onAddItem, userEmail, onLogout, onRefresh, serverStats, currentUser, currentOrgId }: {
    ...
    // Generic Channel Modal (Now only used as part of overview or if needed)
    ...
    // New Channels Overview Modal
    const ChannelsOverviewModal = ({ onClose }: { onClose: () => void }) => {
        return (
                                    ...
                                    <div key={c.channel} className="p-4 rounded-3xl bg-stone-50/50 dark:bg-zinc-800/30 border border-stone-100 dark:border-zinc-800/50">
                                        ...
                                        <div className="h-1.5 bg-stone-200/50 dark:bg-zinc-800/50 rounded-full overflow-hidden">
                                            <div
                                                ...
                                                style={{ width: `${(c.count / displayStats.soldCount) * 100}%` }}
                                            />
                                        </div>
                                    </div>
                                );
    ...
    // Generic Analysis Modal (Overview of all brands)
    const AnalysisModal = ({ type, onClose }: { type: 'margin' | 'profit', onClose: () => void }) => {
        const brands = [...displayStats.topBrands];
                    ...
                    <div className="overflow-y-auto p-6 space-y-3">
                        {sortedBrands.length === 0 ? (
                            ...
                            sortedBrands.map(b => {
                                ...
                                return (
                                    <div key={b.brand} className="flex items-center justify-between p-4 rounded-3xl bg-stone-50/50 dark:bg-zinc-800/30 border border-stone-100 dark:border-zinc-800/50">
                                        <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-right">
                                            <p className={`font-bold ${type === 'profit' ? 'text-stone-900 dark:text-zinc-50' : 'text-stone-400 dark:text-zinc-500 text-sm'}`}>
                                                {formatCurrency(b.profit)}
                                            </p>
                                            <p className={`text-sm font-bold ${type === 'margin' ? 'text-green-600 dark:text-green-400' : 'text-stone-400 dark:text-zinc-500 text-[10px]'}`}>
...

            {
                activeChannel && (
                    activeChannel === 'overview' ? (
                        <ChannelsOverviewModal onClose={() => setActiveChannel(null)} />
                    ) : (
                        <ChannelModal
